...
(() => {
      ...
      grid.innerHTML = items
        ...
        <div class="product card">
          <div style="height:140px;display:flex;align-items:center;justify-content:center">
            <img src="${
              p.image || "https://via.placeholder.com/160x120?text=AirSpot"
          ...
          ${
            p.tag
              ? `<div class="tag tag-${p.tag.toLowerCase()}">${p.tag}</div>`
              : ""
          }
          <div style="margin-top:8px;display:flex;align-items:center;gap:8px">
            <button class="btn add-to-cart" data-id="${p._id}" data-name="${encodeURIComponent(
            p.name
  ...
  AirSpot.handleCheckoutFromPage = async () => {
    const cart = getCart();
    if (!cart.length) {
      AirSpot.toast("Cart empty");
      return;
    }
    const name =
      qs("#custName")?.value.trim() ||
      (JSON.parse(localStorage.getItem(CUSTOMER_KEY) || "null") || {}).name ||
      "Guest";
    const email =
      qs("#custEmail")?.value.trim() ||
      (JSON.parse(localStorage.getItem(CUSTOMER_KEY) || "null") || {}).email ||
      "";
  ...
  AirSpot.userRegisterFromPage = async () => {
    ...
    if (!email) {
      qs("#userMsg").textContent = "Email required";
      return;
    }
    try {
  ...
  AirSpot.workerClock = async (action) => {
    try {
      ...
      if (!token) {
        AirSpot.toast("Please login first");
        return;
      }
  ...
  AirSpot.loadWorkerPanel = async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch("/api/workers/me/hours", {
    ...
    try {
      ...
      qs("#inventory").innerHTML = Array.isArray(items)
        ...
        <div style="display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid rgba(0,0,0,0.04)">
          <div><strong>${i.name}</strong><div class="small">${
                i.description || ""
              }</div></div>
          <div style="text-align:right">
      ...
      console.error(e);
      qs("#inventory").innerHTML =
        "<div class='small'>Inventory unavailable</div>";
    }
  };
